ALTER TABLE public.house_access_logs_raw
  ADD COLUMN IF NOT EXISTS provided_event_id TEXT;

ALTER TABLE public.house_access_logs_raw
  ADD COLUMN IF NOT EXISTS provided_event_name TEXT;

ALTER TABLE public.house_access_logs_raw
  ADD COLUMN IF NOT EXISTS resolved_event_id UUID REFERENCES public.events(id) ON DELETE SET NULL;

ALTER TABLE public.house_access_logs_raw
  ADD COLUMN IF NOT EXISTS resolved_event_name TEXT;

ALTER TABLE public.house_access_logs_raw
  ADD COLUMN IF NOT EXISTS resolution_status TEXT NOT NULL DEFAULT 'pending';

ALTER TABLE public.house_access_logs_raw
  ADD COLUMN IF NOT EXISTS resolution_strategy TEXT;

CREATE INDEX IF NOT EXISTS idx_house_access_logs_raw_resolved_event_id
  ON public.house_access_logs_raw(resolved_event_id);
...
BEGIN
  ...
  FOR event_item IN
    SELECT value
    FROM jsonb_array_elements(input_events)
  LOOP
    ...
    v_occurred_minutes :=
      (EXTRACT(HOUR FROM v_local_timestamp)::INTEGER * 60) +
      EXTRACT(MINUTE FROM v_local_timestamp)::INTEGER;

    v_person_id := NULL;
    v_is_resident := false;
    v_grant_id := NULL;
    v_grant_person_id := NULL;
    v_grant_event_id := NULL;
    v_grant_payload := '{}'::jsonb;
    v_resolved_event_id := NULL;
    v_resolved_event_name := NULL;
    v_resolution_status := 'pending';
    v_resolution_strategy := NULL;

    IF v_house_user_id IS NOT NULL THEN
      SELECT m.person_id, m.is_resident
      INTO v_person_id, v_is_resident
    ...
    IF v_house_user_id IS NOT NULL THEN
      ...
      ORDER BY g.access_start DESC, g.updated_at DESC
      LIMIT 1;
    END IF;
    ...
    IF v_provided_event_id IS NOT NULL
      ...
      WHERE e.id = v_provided_event_id::uuid
      LIMIT 1;

      IF v_resolved_event_id IS NOT NULL THEN
        v_resolution_status := 'resolved';
        v_resolution_strategy := 'provided_event_id';
      END IF;
    END IF;
    ...
    IF v_resolved_event_id IS NULL AND v_grant_event_id IS NOT NULL THEN
      ...
      IF v_resolved_event_name IS NULL THEN
        SELECT e.name
        INTO v_resolved_event_name
        FROM public.events AS e
        WHERE e.id = v_grant_event_id
        LIMIT 1;
      END IF;
    ...
    IF v_resolved_event_id IS NULL THEN
      ...
      IF v_total_events = 0 THEN
        v_resolution_status := 'not_found';
        v_resolution_strategy := 'no_events_on_date';
      ELSIF v_total_events = 1 THEN
        ...
        v_resolution_status := 'resolved';
        v_resolution_strategy := 'single_event_on_date';
      ELSE
        SELECT COUNT(*)
        INTO v_timed_events
        FROM public.events AS e
        WHERE e.date = v_local_date
          AND e.time ~ '^\d{1,2}:\d{2}';

        IF v_timed_events <> v_total_events THEN
          v_resolution_status := 'ambiguous';
          v_resolution_strategy := 'multiple_events_with_unknown_time';
        ELSE
          v_candidate_id := NULL;
          v_candidate_name := NULL;
          v_candidate_minutes := NULL;
          v_candidate_count := 0;
          ...
          FOR rec IN
            SELECT
              ...
              (
                ...
                regexp_replace(split_part(e.time, ':', 2), '[^0-9].*$', '')::INTEGER
              ) AS start_minutes
            FROM public.events AS e
            WHERE e.date = v_local_date
            ORDER BY start_minutes ASC, e.name ASC
          LOOP
            IF rec.start_minutes <= v_occurred_minutes THEN
              IF v_candidate_minutes IS NULL OR rec.start_minutes > v_candidate_minutes THEN
                v_candidate_id := rec.id;
                v_candidate_name := rec.name;
                v_candidate_minutes := rec.start_minutes;
                v_candidate_count := 1;
          ...
          IF v_candidate_id IS NULL THEN
            v_resolution_status := 'not_found';
            v_resolution_strategy := 'before_first_event';
          ELSIF v_candidate_count > 1 THEN
            v_resolution_status := 'ambiguous';
            v_resolution_strategy := 'multiple_events_same_start_time';
          ELSE
            v_resolved_event_id := v_candidate_id;
            v_resolved_event_name := v_candidate_name;
            v_resolution_status := 'resolved';
            v_resolution_strategy := 'latest_started_event';
          END IF;
        END IF;
      END IF;
    END IF;
    ...
    INSERT INTO public.house_access_logs_raw (
      ...
      v_raw_payload,
      v_provided_event_id,
      v_provided_event_name,
      v_resolved_event_id,
      v_resolved_event_name,
      v_resolution_status,
    ...
    IF v_decision = 'granted' AND v_person_id IS NOT NULL THEN
      v_access_type := CASE
        WHEN v_is_resident THEN 'Residente'
        WHEN COALESCE(v_grant_payload ->> 'ticket_type', '') <> '' THEN v_grant_payload ->> 'ticket_type'
        ELSE 'House Access'
      END;
      ...
      IF v_resolved_event_id IS NOT NULL THEN
        ...
        INSERT INTO public.checkins (
          ...
          v_local_date,
          CASE
            WHEN EXTRACT(HOUR FROM v_local_timestamp) < 12 THEN 'Manhã'
            WHEN EXTRACT(HOUR FROM v_local_timestamp) < 18 THEN 'Tarde'
            ELSE 'Noite'
          END,
          v_access_type,
          v_resolved_event_id,
    ...
    v_processed_at := NOW();
    UPDATE public.house_access_logs_raw
